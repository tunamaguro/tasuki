...
pub struct GetAvailableJobsRow {
    ...
    pub j_lease_token: sqlx::types::Uuid,
}
...
impl<'a, Id, LeaseToken> HeartBeatJobBuilder<'a, ((), Id, LeaseToken)> {
    pub fn lease_interval(
        ...
        'a,
        (&'a sqlx::postgres::types::PgInterval, Id, LeaseToken),
    > {
...
> HeartBeatJobBuilder<
    'a,
    (&'a sqlx::postgres::types::PgInterval, sqlx::types::Uuid, Option<sqlx::types::Uuid>),
> {
...
impl<'a, Id, LeaseToken> RetryJobBuilder<'a, ((), Id, LeaseToken)> {
    pub fn interval(
        ...
        'a,
        (Option<&'a sqlx::postgres::types::PgInterval>, Id, LeaseToken),
    > {
        let ((), id, lease_token) = self.fields;
        let _phantom = self._phantom;
        RetryJobBuilder {
...
impl<'a> InsertJobMany<'a> {
    pub const QUERY: &'static str = r"COPY tasuki_job (max_attempts,job_data,queue_name) FROM STDIN (FORMAT BINARY)";
    pub fn query_as(
        &'a self,
    ) -> sqlx::query::QueryAs<
        'a,
...
WHERE
  ...
  AND j.queue_name = $1::TEXT";
        ...
        A: sqlx::Acquire<'b, Database = sqlx::Postgres> + Send + 'a,
    {
        async move {
            let mut conn = conn.acquire().await?;
            sqlx::query(Self::QUERY).bind(self.queue_name).execute(&mut *conn).await
        }
    }
}
impl<'a> RetryFailedByQueue<'a> {
